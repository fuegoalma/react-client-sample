...
- **Contract** (`tests/contract/`) — the client against the vendored `openapi.yaml`, which is **checked against, not generated from** ([ADR 7](docs/adr/0007-openapi-as-a-checked-oracle.md)). `dtoShapes.test.ts` holds every DTO to its schema in both directions through types generated by `openapi-typescript`, and fails when a schema is neither mirrored nor listed as deliberately skipped; `endpoints.test.ts` matches every URL the client issues against the document, in both directions; `listSpecs.test.ts` and `formSchemas.test.ts` do the same for the search forms and the write forms — the latter comparing each Zod field and its length limits with the request body it was transcribed from. **Add a DTO or a form and it must be placed in one of those tables**, or the suite fails.
